        ...
        try:
            ...
            if option==1:
                ...
                try: 
                    ...
                    target = str(input("Ingrese la IP de la que quiera escanear los puertos: "))
                    port_range = str(input("Ingrese el rango de puertos que se desean escanear: "))
                    bash_command = f".\port.sh {target} {port_range}"
                    result = subprocess.run(bash_command, shell=True, capture_output=True, text=True, executable="/bin/bash")
                    print(result.stdout)
                    data = result.stdout
